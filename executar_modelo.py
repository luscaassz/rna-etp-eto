...
def preparar_features(
    ...
    return X, valid_mask, height, width, feature_columns, band_map, mapbiomas_array
...
# =========================================================
# DIAGNÓSTICO E CORREÇÃO FÍSICA DA PREDIÇÃO
# =========================================================
def diagnosticar_pixels_negativos(
    ...
    Isso ajuda a identificar se os outliers estão ligados a alguma classe
    de uso do solo ou a valores extremos nas bandas Landsat.
    """
    ...
    if total_neg == 0:
        log_message(logger, "Nenhum pixel negativo encontrado.")
        return
    ...
    perc = total_neg / total_validos * 100.0 if total_validos > 0 else 0.0

    log_message(logger, "Diagnóstico dos pixels negativos:")
    log_message(logger, f"  Pixels negativos: {total_neg:,} ({perc:.4f}%)")
    ...
    if usos.size > 0:
        classes, counts = np.unique(usos.astype(int), return_counts=True)
        ordem = np.argsort(counts)[::-1]

        log_message(logger, "  Classes MapBiomas nos pixels negativos:")

        for idx in ordem[:10]:
            classe = classes[idx]
            qtd = counts[idx]
            pct = qtd / total_neg * 100.0 if total_neg > 0 else 0.0
            log_message(
                logger,
...
def preencher_outliers_por_mediana(
    ...
    Não usa outros pixels inválidos como vizinhos válidos.
    """
...
def corrigir_predicao_fisica_et(
    ...
    Regras:
    - ET negativa não é fisicamente válida.
    - Valores acima de et_max são tratados como outliers.
    - Pequenos buracos podem ser preenchidos pela mediana dos vizinhos.
    """
    ...
    log_message(logger, "Correção física da predição:")
    log_message(logger, f"  ET mínimo permitido: {et_min:.2f}")
    log_message(logger, f"  ET máximo permitido: {et_max:.2f}")
    log_message(logger, f"  Pixels válidos antes: {total_validos:,}")

    perc = total_incoerentes / total_validos * 100.0 if total_validos > 0 else 0.0
    log_message(
        logger,
        f"  Pixels incoerentes encontrados: {total_incoerentes:,} ({perc:.4f}%)",
    )
...
def executar_modelo(
    ...
    try:
        ...
        X, valid_mask, height, width, _, band_map, mapbiomas_proc = preparar_features(
        ...
        diagnosticar_pixels_negativos(
            resultado=resultado,
            band_map=band_map,
            mapbiomas_array=mapbiomas_proc,
            logger=logger,
        )
